...
async function initializePool(): Promise<Pool> {
  ...
  console.log(`PostgreSQL Pool: Value of process.env.POSTGRES_URL: '${connectionString}' (Type: ${typeof connectionString})`);
  ...
  if (!pool) {
    try {
      ...
      pool.on('error', (err, client) => {
        ...
        pool = null; // Reset pool so it attempts to reinitialize on next call
      });
    } catch (err) {
      console.error("PostgreSQL Pool (getPool): Error during initializePool call:", (err as Error).message);
      // poolInitializationError is already set by initializePool in this case
